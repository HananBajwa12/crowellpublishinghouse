...
export async function POST(req: Request) {
  try {
    ...
    const { client_name, client_email, amount, currency, description } = body;
...
console.log("SECRET:", paypalClientSecret?.slice(0, 8));
console.log("BASE URL:", paypalBaseUrl);
    ...
    const { error: dbError } = await supabaseAdmin.from("payment_links").insert({
      client_name,
      client_email,
